...
const SIPAY_ENDPOINT = "/api/recurringPlan/query";
...
export async function POST(request: Request) {
  try {
    ...
    if (!merchantKey) {
      return NextResponse.json(
        { error: 'SIPAY_MERCHANT_KEY environment variable is not configured' },
        { status: 500 }
      );
    }
    ...
    if (!body.plan_code || !body.recurring_number) {
      return NextResponse.json({
        message: "Required fields are missing: plan_code and recurring_number are required"
      }, { status: 400 });
    }
    ...
    console.error("Sipay recurring query error:", error);
    return NextResponse.json({
